...
#  Lancer : streamlit run app_oracle_nlp_int_pro.py
# ============================================================
...
def post_process_sql(sql: str, question: str) -> str:
    ...
    # Le modèle génère parfois '1' au lieu du nom d'action textuel
    ...
    if re.search(r"OBJ_NAME\s*=\s*'1'", sql, re.IGNORECASE):
        ...
        for obj in known_objects:
            if obj.lower() in q:
                ...
                break
        if not found_obj:
            # Chercher un mot en MAJUSCULES dans la question originale
...
def clean_sql(raw: str, question: str = "") -> str:
    ...
    sql = raw.strip().split("\n\n")[0].strip()
    # Nettoyer tout HTML résiduel que le modèle pourrait générer
    sql = re.sub(r'<[^>]+>', '', sql)
    sql = sql.strip()
...
def df_to_html_table(df: pd.DataFrame) -> str:
    """Génère une table HTML propre depuis un DataFrame, sans inject de HTML externe"""
    if df is None or df.empty:
...
def load_phi3():
    ...
    return Llama(
        ...
        n_ctx        = 2048,
        n_gpu_layers = 0,   # CPU only — stable Windows sans CUDA llama.cpp
        verbose      = False,
    )
...
with tab_chat:
    ...
    with col_main:
        ...
        if not st.session_state.historique:
            ...
            for msg in st.session_state.historique:
                ...
                if r == "user":
                    ...
                    # ═══ CORRECTION PRINCIPALE : SQL affiché dans <pre> — JAMAIS de highlight_sql() ═══
                    # Le contenu c est du texte SQL brut, échappé pour HTML
                    c_escaped = c.replace("&","&amp;").replace("<","&lt;").replace(">","&gt;")
                    html_msgs += f"""
                    <div style="margin-left:38px;margin-top:-4px">
                      <div class="qf-sql-wrap">
                        <div class="qf-sql-bar">
                          <span>SQL Oracle · TinyLlama LoRA V2</span>
                    ...
                    # c contient du HTML de table généré par df_to_html_table()
                    html_msgs += f"""
        ...
        if send and user_input.strip():
            ...
            # Stocké en texte brut — la mise en forme se fait au rendu via <pre>
...
with tab_admin:
    ...
    with adm2:
        ...
        <div class="qf-card">
          ...
          <div class="qf-comp-row"><div><div class="qf-comp-name">LoRA V2.1</div><div class="qf-comp-sub">r=32 · alpha=64 · 5 epochs · 4500 ex.</div></div>{badge(lora_ok)}</div>
          ...
          <div class="qf-comp-row"><div><div class="qf-comp-name">Dataset entraînement</div><div class="qf-comp-sub">4500 exemples V2.1</div></div>{badge(ds_ok)}</div>
        </div>
        <div class="qf-card">
          ...
          <div class="qf-config-grid">
            ...
            <div class="qf-cfg"><div class="qf-cfg-lbl">SQL→FR</div><div class="qf-cfg-val" style="color:var(--green)">64%</div></div>
            <div class="qf-cfg"><div class="qf-cfg-lbl">Global</div><div class="qf-cfg-val" style="color:var(--green)">82%</div></div>
