...
## 34. Préparation de la comparaison Qwen3-4B et Gemma 3 4B — 14 septembre 2026

La comparaison demandée est engagée avec deux modèles Q4 de taille comparable : Qwen3-4B-Q4_K_M, 2 497 280 256 octets, et Gemma 3 4B IT Q4_K_M, 2 489 757 856 octets. Les fichiers de poids restent exclus de Git. Le téléchargeur scripts/download-comparison-model.py utilise 32 segments reprenables, vérifie la taille complète puis l’empreinte SHA-256 publiée par le dépôt avant de rendre le fichier final disponible.

scripts/start-local.ps1 accepte désormais les profils qwen3 et gemma3, en plus de light et quality. Un seul profil est chargé à la fois. Qwen3 reçoit le commutateur /no_think afin de réserver ses tokens au plan JSON. Gemma reçoit le même contrat sémantique dans un message utilisateur unique pour respecter son format de dialogue. Le profil auto choisira Qwen3 seulement lorsque son fichier final vérifié existe.

Le benchmark v4 mesure désormais deux niveaux séparés : le plan JSON brut produit par le modèle et le plan final après validation de sécurité. Cette séparation permettra de déterminer si un bon score vient réellement de la compréhension du modèle ou des corrections déterministes. scripts/run-model-comparison.ps1 exécute une présélection de treize cas figés couvrant temps, classement, catalogue, filtres, comptage distinct, comparaison, récence, refus et ambiguïté. L’option Full conserve les cinquante cas.

Au checkpoint, les téléchargements ont été passés en mode séquentiel à cause des coupures du CDN : Qwen3 termine en premier, tandis que les segments Gemma déjà reçus sont conservés. Après validation de Qwen3, la présélection sera exécutée, Gemma reprendra ensuite, puis les deux résultats seront comparés sans modifier le corpus.
