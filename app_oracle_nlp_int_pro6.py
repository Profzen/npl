...
# Build users and tables lists from real audit data when available (no fake data in prod)
USERS_REELS = []
TABLES_REELLES = []
if not audit_df.empty:
    if "DBUSERNAME" in audit_df.columns:
        usr_counts = audit_df["DBUSERNAME"].value_counts().to_dict()
        USERS_REELS = [{"name": k, "actions": int(v)} for k, v in usr_counts.items()]
    if "OBJECT_NAME" in audit_df.columns:
        obj_counts = audit_df["OBJECT_NAME"].value_counts().to_dict()
        TABLES_REELLES = [{"name": k, "actions": int(v)} for k, v in obj_counts.items()]
...
# Recent actions: derive from audit_df if available, otherwise leave empty (no fake data)
recent_html = ""
if not audit_df.empty and {"DBUSERNAME", "EVENT_TIMESTAMP"}.issubset(set(audit_df.columns)):
    recent_rows = audit_df.sort_values("EVENT_TIMESTAMP", ascending=False).head(10)
    items = []
    for _, r in recent_rows.iterrows():
        user = r.get("DBUSERNAME", "")
        obj = r.get("OBJECT_NAME", "") if pd.notna(r.get("OBJECT_NAME", None)) else ""
        date = str(r.get("EVENT_TIMESTAMP", ""))
        items.append(
            "<li style=\"flex-direction:column;align-items:flex-start;gap:1px;padding:8px 10px\">"
            + "<span style=\"font-size:12px;font-weight:700;color:#2c3d57\">" + str(user) + "</span>"
            + ("<span style=\"font-size:11px;color:#5f6f87\">&rarr; " + str(obj) + "</span>" if obj else "")
            + "<span style=\"font-size:10px;color:#a0b0cc\">" + str(date) + "</span>"
            + "</li>"
        )
    recent_html = "".join(items)
