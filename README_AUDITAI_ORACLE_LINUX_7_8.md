...
### 24.7 Déploiement universel Oracle Linux 7.9 et Oracle Linux 8.x

#### 24.7.1 Stratégie retenue

La compatibilité entre Oracle Linux 7.9 et Oracle Linux 8.x est assurée par des images OCI contenant leur propre espace utilisateur récent :

```text
Hôte Oracle Linux 7.9
└── Oracle Container Runtime for Docker
    ├── auditai-backend : Python 3.11 + FastAPI + modèles Python
    ├── auditai-frontend : Node.js 20 + Next.js
    └── auditai-proxy : Nginx

Hôte Oracle Linux 8.x
└── Podman
    ├── auditai-backend : même image
    ├── auditai-frontend : même image
    └── auditai-proxy : même image
```

Les modèles ne sont pas copiés dans l’image backend. Ils restent dans `/opt/auditai` et sont montés en lecture seule. Cette organisation permet de remplacer le LoRA sans reconstruire les images.

Le déploiement natif direct n’est pas le chemin retenu sur Oracle Linux 7.9, car les versions système de Python, OpenSSL, GCC, glibc et Node.js sont trop anciennes pour constituer une base reproductible pour Python 3.11, Node.js 20 et les bibliothèques NLP actuelles.

Oracle Linux 7 étant en support étendu, une migration vers Oracle Linux 8 ou 9 doit rester planifiée. Le mode conteneur permet néanmoins d’exécuter cette version d’intégration sur l’hôte existant.

#### 24.7.2 Fichiers de déploiement fournis
...
├── start-auditai.sh
├── stop-auditai.sh
└── auditai-containers.service
```

Ces fichiers sont fournis dans le paquet `auditai_oracle_linux_deployment_pack.zip`.

#### 24.7.3 Précontrôle commun
...
```text
Oracle Linux 7.9 ou Oracle Linux 8.x
Architecture x86_64
16 Go de RAM minimum pratique
25 Go libres minimum
SELinux Enforcing ou Permissive, mais pas désactivé sans validation sécurité
```

Tester le serveur Oracle :

```bash
nc -vz <ORACLE_HOST> <ORACLE_PORT>
```

Ne jamais utiliser `127.0.0.1` comme hôte Oracle si la base se trouve sur l’hôte physique : depuis un conteneur, `127.0.0.1` désigne le conteneur lui-même. Utiliser le DNS ou l’adresse IP réelle du serveur Oracle.

#### 24.7.4 Oracle Linux 7.9 : installation du moteur Docker Oracle
...
Oracle Container Runtime for Docker sur Oracle Linux 7 requiert un noyau UEK adapté. Utiliser UEK R5 ou version ultérieure, UEK R6 étant recommandé sur Oracle Linux 7.9.

Installer les utilitaires yum :

```bash
sudo yum install -y yum-utils
```

Activer le dépôt Oracle Linux Addons :

```bash
sudo yum-config-manager --enable ol7_addons
```

Vérifier l’éventuelle présence de l’ancien paquet Docker :
...
Installer Oracle Container Runtime for Docker :
...
Ajouter le compte d’exploitation au groupe Docker seulement si la politique de sécurité l’autorise :

```bash
sudo usermod -aG docker aziz
```

Une nouvelle connexion SSH est nécessaire pour prendre en compte ce groupe. La procédure fournie utilise néanmoins `sudo` afin de rester explicite.

Vérifier le stockage Docker :

```bash
sudo docker info | grep -E 'Storage Driver|Backing Filesystem|Supports d_type'
```

Si l’hôte utilise XFS sans `ftype=1`, éviter `overlay2` sur ce système de fichiers. Utiliser un volume dédié compatible ou la configuration de stockage validée par l’administrateur système. Ne pas modifier le stockage Docker d’un serveur existant sans sauvegarde.

#### 24.7.5 Oracle Linux 8.x : installation de Podman
...
Podman exécute les mêmes Dockerfiles. Les scripts de déploiement détectent automatiquement `podman`, puis `docker`.

Pour fournir la commande `docker` comme alias de Podman, cette installation est facultative :

```bash
sudo dnf install -y podman-docker
```

Le paquet n’est pas requis par les scripts fournis.

#### 24.7.6 Installation des outils communs

Oracle Linux 7 :

```bash
sudo yum install -y git curl jq unzip tar gzip firewalld policycoreutils-python
```

Oracle Linux 8 :

```bash
sudo dnf install -y git curl jq unzip tar gzip firewalld policycoreutils-python-utils
```
...
#### 24.7.7 Installation du projet

Créer les répertoires :

```bash
sudo mkdir -p /opt/auditai
sudo mkdir -p /opt/auditai/runtime
sudo mkdir -p /etc/auditai
sudo mkdir -p /var/backups/auditai
```
...
Si l’archive contient un dossier racine supplémentaire, déplacer son contenu afin d’obtenir :

```text
/opt/auditai/backend
/opt/auditai/frontend
/opt/auditai/TinyLlama-1.1B-Chat-v1.0
/opt/auditai/tinyllama_oracle_lora
/opt/auditai/phi3-mini-gguf
/opt/auditai/deploy/oracle-linux
```
...
sudo useradd \
  --system \
  --home-dir /opt/auditai \
  --shell /sbin/nologin \
  auditai 2>/dev/null || true

sudo chown -R auditai:auditai /opt/auditai
sudo chmod 750 /opt/auditai
```

Les commandes de construction des images doivent pouvoir lire les sources. Les modèles restent montés en lecture seule au moment de l’exécution.

#### 24.7.8 Vérification des fichiers indispensables

```bash
sudo test -f /opt/auditai/backend/app/main.py
sudo test -f /opt/auditai/backend/requirements.txt
sudo test -f /opt/auditai/frontend/package.json

sudo test -f /opt/auditai/TinyLlama-1.1B-Chat-v1.0/model.safetensors
sudo test -f /opt/auditai/tinyllama_oracle_lora/adapter_config.json
sudo test -f /opt/auditai/tinyllama_oracle_lora/adapter_model.safetensors
sudo test -f /opt/auditai/phi3-mini-gguf/Phi-3-mini-4k-instruct-q4.gguf

sudo test -f /opt/auditai/deploy/oracle-linux/Dockerfile.backend
sudo test -f /opt/auditai/deploy/oracle-linux/Dockerfile.frontend
sudo test -f /opt/auditai/deploy/oracle-linux/start-auditai.sh

echo "Structure Audit AI valide"
```
...
```bash
sha256sum \
  /opt/auditai/TinyLlama-1.1B-Chat-v1.0/model.safetensors \
  /opt/auditai/tinyllama_oracle_lora/adapter_model.safetensors \
  /opt/auditai/phi3-mini-gguf/Phi-3-mini-4k-instruct-q4.gguf
```

#### 24.7.9 Configuration

Copier le modèle :

```bash
sudo cp \
  /opt/auditai/deploy/oracle-linux/auditai.env.example \
  /etc/auditai/auditai.env
```

Modifier les valeurs :

```bash
sudo vi /etc/auditai/auditai.env
```
...
ORACLE_USER=AUDIT_AI_READONLY
ORACLE_PASSWORD=CHANGE_ME
ORACLE_HOST=oracle-db.internal
ORACLE_PORT=1521
ORACLE_SERVICE=ORCLPDB1
ORACLE_TABLE=SMART2DSECU.UNIFIED_AUDIT_DATA
...
PHI3_PATH=/models/phi3/Phi-3-mini-4k-instruct-q4.gguf

BACKEND_CORS_ORIGINS=http://auditai.internal
MAX_CONCURRENT_QUERIES_PER_USER=1
MAX_SQL_TOKENS=256
ORACLE_POOL_MIN=1
ORACLE_POOL_MAX=5
...
```bash
sudo chown root:root /etc/auditai/auditai.env
sudo chmod 600 /etc/auditai/auditai.env
```

Supprimer un ancien fichier runtime contenant des chemins Windows ou des secrets :

```bash
sudo rm -f /opt/auditai/backend_runtime_settings.json
```

#### 24.7.10 Construction des images

Rendre les scripts exécutables :

```bash
sudo chmod 750 /opt/auditai/deploy/oracle-linux/*.sh
```

Lancer le script universel :

```bash
sudo /opt/auditai/deploy/oracle-linux/start-auditai.sh --build
```

Le script :

1. détecte Podman ou Docker ;
2. crée le réseau `auditai-net` ;
3. construit `auditai-backend:v1` ;
4. construit `auditai-frontend:v1` avec `/api` comme URL backend ;
5. récupère l’image Nginx ;
6. démarre les trois conteneurs ;
7. publie uniquement les ports 80 et éventuellement 443 ;
8. monte les modèles en lecture seule ;
9. monte `/opt/auditai/runtime` pour SQLite et les données runtime.

Suivre la construction :

```bash
sudo podman ps -a 2>/dev/null || sudo docker ps -a
```

Logs :

```bash
sudo podman logs -f auditai-backend 2>/dev/null || \
sudo docker logs -f auditai-backend
```

#### 24.7.11 Déploiement hors ligne

Sur une machine connectée, construire puis exporter les images :

```bash
cd /opt/auditai

docker build \
  -f deploy/oracle-linux/Dockerfile.backend \
  -t auditai-backend:v1 .

docker build \
  --build-arg NEXT_PUBLIC_API_BASE_URL=/api \
  -f deploy/oracle-linux/Dockerfile.frontend \
  -t auditai-frontend:v1 .
...
docker save \
  ...
  auditai-frontend:v1 \
  nginx:stable-alpine |
gzip > auditai-container-images-v1.tar.gz
```

Transférer `auditai-container-images-v1.tar.gz` sur le serveur.

Oracle Linux 7 :

```bash
gunzip -c auditai-container-images-v1.tar.gz |
sudo docker load
```
...
Puis démarrer sans reconstruction :

```bash
sudo /opt/auditai/deploy/oracle-linux/start-auditai.sh
```

#### 24.7.12 Démarrage automatique

Installer l’unité générique :

```bash
sudo cp \
  /opt/auditai/deploy/oracle-linux/auditai-containers.service \
  /etc/systemd/system/auditai-containers.service

sudo systemctl daemon-reload
sudo systemctl enable --now auditai-containers
```

Vérifier :

```bash
sudo systemctl status auditai-containers --no-pager
```

#### 24.7.13 Pare-feu et SELinux
...
Ne pas ouvrir les ports 3000 et 8000 : ils restent dans le réseau conteneur.

Les volumes utilisent le suffixe SELinux `:Z`. Vérifier les refus :

```bash
sudo ausearch -m AVC,USER_AVC -ts recent
```

Ne pas désactiver SELinux pour contourner un problème de permission.

#### 24.7.14 Contrôles après démarrage

Liste des conteneurs :

```bash
sudo podman ps 2>/dev/null || sudo docker ps
```
...
```bash
sudo podman logs --tail 200 auditai-backend 2>/dev/null || \
sudo docker logs --tail 200 auditai-backend
```
...
Authentification et santé :

```bash
read -p "Utilisateur Audit AI : " AUDITAI_LOGIN
read -s -p "Mot de passe Audit AI : " AUDITAI_PASSWORD
echo
...
TOKEN=$(
  curl -sS \
    ...
    -d "{\"username\":\"${AUDITAI_LOGIN}\",\"password\":\"${AUDITAI_PASSWORD}\"}" \
...
curl -sS \
  ...
  -H "Content-Type: application/json" \
  -H "X-Auth-Token: $TOKEN" \
  -d '{"question":"Quelle est la dernière action effectuée par SYSTEM ?"}' \
  http://127.0.0.1/api/query |
jq
```

Le SQL doit utiliser `DBUSERNAME` et rester une requête de lecture.

#### 24.7.15 Arrêt et redémarrage

Arrêt :

```bash
sudo /opt/auditai/deploy/oracle-linux/stop-auditai.sh
```
...
Reconstruction après modification du code ou des dépendances :

```bash
sudo /opt/auditai/deploy/oracle-linux/start-auditai.sh --build
```

#### 24.7.16 Mise à jour du LoRA conteneurisé
...
sudo mv \
  /opt/auditai/tinyllama_oracle_lora \
  /opt/auditai/tinyllama_oracle_lora_backup

sudo mv \
  /tmp/tinyllama_oracle_lora \
  /opt/auditai/tinyllama_oracle_lora

sudo chown -R auditai:auditai \
  /opt/auditai/tinyllama_oracle_lora

sudo /opt/auditai/deploy/oracle-linux/start-auditai.sh
```

Aucune reconstruction d’image n’est nécessaire : le LoRA est monté depuis l’hôte.

#### 24.7.17 Sauvegarde

Arrêter le backend avant la sauvegarde SQLite :
...
```bash
sudo tar -czf \
  "/var/backups/auditai/auditai-$(date +%Y%m%d-%H%M%S).tar.gz" \
  /etc/auditai/auditai.env \
  /opt/auditai/runtime \
  /opt/auditai/tinyllama_oracle_lora \
  /opt/auditai/deploy/oracle-linux
...
#### 24.7.18 Limites et décision d’exploitation

- Oracle Linux 7.9 est supporté ici par conteneurisation, pas comme cible native des runtimes récents.
- L’hôte Oracle Linux 7 doit utiliser un noyau et un stockage compatibles avec Oracle Container Runtime for Docker.
- Oracle Linux 7 est en support étendu ; une migration reste recommandée.
- Sur Oracle Linux 8, Podman est le moteur recommandé.
- Le serveur doit disposer d’environ 25 Go libres avant la première construction.
- L’accès à Internet n’est pas obligatoire si les images sont construites et exportées ailleurs.
- Les modèles restent locaux et ne sont jamais envoyés vers une API externe.
...
### Compatibilité Oracle Linux

- [ ] version Oracle Linux détectée
- [ ] Oracle Linux 7.9 utilise Oracle Container Runtime for Docker
- [ ] noyau UEK compatible vérifié sur Oracle Linux 7
- [ ] Oracle Linux 8 utilise Podman
- [ ] au moins 25 Go libres avant construction
- [ ] trois conteneurs actifs
- [ ] seuls les ports 80/443 sont exposés
- [ ] modèles montés en lecture seule
- [ ] procédure hors ligne testée si le serveur n’a pas Internet
